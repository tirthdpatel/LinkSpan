...
### Decision: **WebRTC DTLS Only (No additional application-layer encryption)**
...
2. **No intermediate server can read data.** Even when TURN relay is used, DTLS encryption means the TURN server only sees encrypted bytes — it cannot read file contents.
